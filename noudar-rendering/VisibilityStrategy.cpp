...
namespace odb {
    ...
    bool VisibilityStrategy::isBlock(const IntMap &occluders, Knights::EDirection direction,
    ...
    void
    VisibilityStrategy::castVisibility(VisMap &visMap, const IntMap &occluders, Knights::Vec2i pos,
    ...
    bool VisibilityStrategy::isVisibleAt(const VisMap &visMap, Knights::EDirection from,
                                         Knights::Vec2i currentPos) {
        auto converted = transform(from, currentPos);
        return visMap[converted.y][converted.x] == EVisibility::kVisible;
    }

    void VisibilityStrategy::setIsVisible(VisMap &visMap, Knights::EDirection from,
                                          Knights::Vec2i currentPos) {
        auto converted = transform(from, currentPos);
    ...
    Knights::Vec2i
    VisibilityStrategy::transform(Knights::EDirection from, Knights::Vec2i currentPos) {
    ...
    void VisibilityStrategy::castVisibility(Knights::EDirection from, VisMap &visMap,
                                            const IntMap &occluders,
        ...
        while (stackPos > 0) {
            ...
            if ((!kNarrowByDistance || (currentPos.x - originalPos.x) >= -std::abs(narrowing)) &&
                (currentPos.x - originalPos.x) <= 0 && stackPos < positions.size() - 1) {
                positions[stackPos] = Knights::Vec2i{currentPos.x + leftOffset.x,
                                                     currentPos.y + leftOffset.y};
                ++stackPos;
            }

            if ((!kNarrowByDistance || (currentPos.x - originalPos.x) <= std::abs(narrowing)) &&
                (currentPos.x - originalPos.x) >= 0 && stackPos < positions.size() - 1) {
                positions[stackPos] = Knights::Vec2i{currentPos.x + rightOffset.x,
                                                     currentPos.y + rightOffset.y};
                ++stackPos;
            }

            if (distance <= 0 && stackPos < positions.size() - 1) {
                auto mapOffset = Knights::mapOffsetForDirection(Knights::EDirection::kNorth);
                positions[stackPos] = Knights::Vec2i{currentPos.x + mapOffset.x,
                                                     currentPos.y + mapOffset.y};
                ++stackPos;
            }
