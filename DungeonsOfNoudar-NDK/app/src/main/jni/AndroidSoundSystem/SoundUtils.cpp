...
namespace odb {
	std::shared_ptr<SoundClip> makeSoundClipFrom(std::shared_ptr<AudioSink> audioSink, FILE* source) {
		...
		printf("Frequency: %u\n", frequency);
		printf("Bits: %u\n", bits);
		return std::make_shared<SoundClip>( audioSink, buffer, size, offset, bits, channels, frequency );
	}
}