...
const unsigned OPL2::fIntervals[8] = {
  48, 95, 190, 379, 759, 1517, 3034, 6069
};
const unsigned OPL2::notes[12] = {
  26163, 27718,		// C, C#
  29366, 31113,		// D, D#
  32963,				// E
  34923, 36999,		// F, F#
...
const byte OPL2::drumOffset[6] = {
  0x10, 0x13, 0x14, 0x12, 0x15, 0x11
};
const byte OPL2::instrumentBaseRegs[11] = {
...
short OPL2::getNoteFrequency(byte channel, byte octave, byte note) {
	...
	unsigned interval = fIntervals[getBlock(channel)];
	unsigned long freq = notes[note % 12] * 10UL;
	...
	short result = (short)max(0, min(freq / interval, 1023));
	return result;
}
